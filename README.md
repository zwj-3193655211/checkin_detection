...
| 项目 | 内容 |
|---|---|
| 项目名称 | 晨读晨练签到打卡检测系统（checkin_detection） |
| 时间 | 2026.07 – 2026.08 |
| 角色 | 算法设计 / 开发 |
| 指导教师 | 邵国林 |
| 开源地址 | https://github.com/zwj-3193655211/checkin_detection |

## 项目简介

基于 CLIP ViT-B/32 提取 512 维视觉-语言特征（免从头训练），设计双 MLP 架构——主分类器（晨读/晨跑二分类）+ 特征预测器（11 维可解释特征），将复杂判定拆解为要素识别；引入温度缩放（T=5.0）平滑置信度、缓解过度自信；构建四规则三支决策系统，结合置信度与特征匹配数科学降低漏检与人工审核负担。

在约 2057 张标注数据上实现漏检率 0.00%、分类准确率约 99%、人工审核率约 24%；已在 GitHub 开源（github.com/zwj-3193655211/checkin_detection）。

> 现已扩展为 **CLIP / SigLIP2-NaFlex-B/16 双编码器架构**，可在 GUI 中一键热切换，详见下方「近期工程优化」。
...
1. **逐维度阈值调优 + 安全下限**
   - 放弃「单一全局 floor」，改为对 11 个特征维度各自以 Youden's J（TPR − FPR）独立寻优，让「打了标签的图更易通过、没打标签的图更不易通过」。
   - 叠加用户硬性要求的 **0.60 安全下限**：平均维度准确率仍保持 95.79%（远高于默认 88.44%），漏检恢复为 0.00%。

2. **双编码器架构 + UI 热切换**
   - 编码器抽象为统一工厂（CLIP ViT-B/32 512 维 / SigLIP2-NaFlex-B/16 768 维，原生宽高比零裁切）。
   - GUI 顶部「识别模型」下拉框可即时切换 CLIP / SigLIP，切换后自动重载编码器与双 MLP 头，并动态刷新标题、状态栏、参数栏与各维度阈值策略。

3. **修复切换大模型时的窗口卡死**
   - 根因：原 `_on_encoder_change` 在主线程同步加载 SigLIP 大模型，阻塞 Tkinter 事件循环被判「未响应」。
...
- **免训练特征提取**：复用预训练 CLIP / SigLIP 视觉编码器，仅训练轻量双 MLP 头，成本低、可复现。
...
- **双编码器热切换**：CLIP 与 SigLIP 运行时无缝切换，便于对比与部署。
...
输入图片
   │
   ▼
[CLIP / SigLIP 视觉编码器]  → 512 / 768 维图像特征
   │
...
- **主分类器**：输入 512/768 维特征，隐藏 256，输出 2（晨读/晨跑），softmax 前除以 `CLASSIFIER_TEMPERATURE=5.0`。
- **特征预测器**：输入 512/768 维特征，隐藏 512，输出 11，推理时已含 sigmoid + 温度缩放（`FEATURE_TEMPERATURE=1.8`）。
...
| **推荐：SigLIP / CLIP + 逐维度阈值 + 0.60 下限** | **14.95%** | **85.05%** | **0.00%** |

> 漏检（异常被放行）为一票否决项，必须 = 0%。
> 简介中的「人工审核率约 24%」为综合/保守口径表述，上表为推荐配置（0.60 下限）口径，两者统计口径不同。

## 快速开始

### 方式一：一键启动（推荐）

直接双击根目录的 `launch.bat`，自动调用专属 conda 环境 `checkin_detection` 运行 GUI。
...
│   ├── encoders.py         # CLIP / SigLIP / ViT-Tiny 编码器工厂
